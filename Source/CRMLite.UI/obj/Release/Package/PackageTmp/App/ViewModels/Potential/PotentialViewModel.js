function PotentialViewModel() {
    ...
    self.selectedAminity = ko.observable(new Aminities({}));
    ...
    self.createAminity = function () {
        if (self.selectedAminity().modelState.isValid()) {
            var aminityJson = ko.toJS(self.selectedAminity());
            alert(ko.toJS(self.url.potentialManageCreateAminity));
            var result = propznetSuite.dataManager.postData(ko.toJS(self.url.potentialManageCreateAminity), aminityJson);
            result.done(function (data) {
                if (data) {
                    $('#createAminityForProperty').modal('hide');
                    self.aminityList.push(new Aminities(aminityJson));
                }
                else {
            ...
            self.selectedAminity().showErrors();
        }
    };
    ...
    self.saveMorePotential = function () {
        if (self.modelState.isValid()) {
            ...
            var result = propznetSuite.dataManager.postData(ko.toJS(self.url.potentialCreateMorePotential), potentialJson);
            result.done(function (data) {
...
//            var result = propznetSuite.dataManager.postData(ko.toJS(self.url.propertyManageCreateAminity), aminityJson);
//            result.done(function (data) {
//                if (data) {
//                    $('#createAminityForProperty').modal('hide');
//                    self.aminityList.push(new Aminities(aminityJson));
